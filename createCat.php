...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($cat_name_err) && empty($breed_err) && empty($cat_color_err) && empty($gender_err) && empty($origin_err) && empty($age_in_years_err) && empty($Capture_date_err) && empty($Supplier_id_err) && empty($food_supplied_err)){
        ...
        $sql = "INSERT INTO Cat_info (cat_name, breed, cat_color, gender, origin, age_in_years, Capture_date, Supplier_id, food_supplied) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssisis", $param_name, $param_breed, $param_cat_color, $param_gender, $param_origin, $param_age_in_years, $param_Capture_date, $param_Supplier_id, $param_food_supplied);
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <p>Please fill this form and submit to add new Wild Cat record to the database.</p>
